...
2. 必须使用Unity**2022.3.0**(注意其它版本都有各种问题，请务必使用该版本)  
...
# WebGL运行
1. 注意webgl hybrid安装要用全局模式 参考 https://hybridclr.doc.code-philosophy.com/docs/basic/buildwebgl  
...
4. copy ET/Config/IIS/web.config到ET/Release目录，这是配置IIS的配置  
5. 用微信开发者工具导入ET/Release/minigame目录,运行  
6. 注意WebGL模式Unity并没有实现完整的Wesocket，所以无法使用ClientServer模式，请单独运行服务器
